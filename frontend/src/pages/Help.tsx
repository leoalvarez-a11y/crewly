...
export const Help: React.FC = () => {
  ...
  return <div className="max-w-5xl mx-auto space-y-6">
    ...
    <a className="flex items-center gap-2 text-primary" href="/docs/es/GUIA_INTERACTIVA.html" target="_blank" rel="noreferrer"><BookOpen className="w-5 h-5" />Guia interactiva con imagenes</a>
    <div className="grid md:grid-cols-2 gap-3">{GUIDES.map((guide) => <a key={guide} href={`/help/guide/${guide}`} className="rounded-lg border border-border-dark p-4 hover:border-primary"><span className="font-medium">{guide.replace(/-/g, ' ')}</span></a>)}</div>
    <a className="block text-primary" href="/help/guide/COSTOS-Y-SUSCRIPCIONES">Costos y suscripciones</a>
  </div>;
};
