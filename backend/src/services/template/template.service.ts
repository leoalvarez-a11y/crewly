    ...
    for (const role of template.roles) {
      ...
      for (let i = 0; i < role.count; i++) {
        ...
        const member: TeamMember = {
          ...
          modelSelectionMode: role.modelId ? 'manual' : role.capabilityClass ? 'automatic' : undefined,
          capabilityClass: role.capabilityClass,
          optionalFallbackModel: role.optionalFallbackModel,
          memoryLayerEnabled: role.memoryLayerEnabled,
