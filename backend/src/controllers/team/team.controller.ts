  ...
  try {
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      const teamMember: TeamMember = {
        ...
        ...(member.modelSelectionMode ? { modelSelectionMode: member.modelSelectionMode } : {}),
        ...(member.capabilityClass ? { capabilityClass: member.capabilityClass } : {}),
        ...(member.optionalFallbackModel ? { optionalFallbackModel: member.optionalFallbackModel } : {}),
        ...(member.optionalBudget ? { optionalBudget: member.optionalBudget } : {}),
        ...(member.memoryLayerEnabled !== undefined ? { memoryLayerEnabled: member.memoryLayerEnabled } : {}),
      ...
      team.members = await Promise.all(updates.members.map(async (memberUpdate: TeamMemberUpdate) => {
        ...
        if (existingMember) {
          return {
            ...
            modelId: memberUpdate.modelId ?? existingMember.modelId,
            provider: memberUpdate.provider ?? existingMember.provider,
            modelSelectionMode: memberUpdate.modelSelectionMode ?? existingMember.modelSelectionMode,
            capabilityClass: memberUpdate.capabilityClass ?? existingMember.capabilityClass,
            optionalFallbackModel: memberUpdate.optionalFallbackModel ?? existingMember.optionalFallbackModel,
            optionalBudget: memberUpdate.optionalBudget ?? existingMember.optionalBudget,
            memoryLayerEnabled: memberUpdate.memoryLayerEnabled ?? existingMember.memoryLayerEnabled,
          ...
          return {
            ...
            ...(memberUpdate.modelId ? { modelId: memberUpdate.modelId } : {}),
            ...(memberUpdate.provider ? { provider: memberUpdate.provider } : {}),
            ...(memberUpdate.modelSelectionMode ? { modelSelectionMode: memberUpdate.modelSelectionMode } : {}),
            ...(memberUpdate.capabilityClass ? { capabilityClass: memberUpdate.capabilityClass } : {}),
            ...(memberUpdate.optionalFallbackModel ? { optionalFallbackModel: memberUpdate.optionalFallbackModel } : {}),
            ...(memberUpdate.optionalBudget ? { optionalBudget: memberUpdate.optionalBudget } : {}),
            ...(memberUpdate.memoryLayerEnabled !== undefined ? { memoryLayerEnabled: memberUpdate.memoryLayerEnabled } : {}),
